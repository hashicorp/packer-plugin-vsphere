...
The [temporary_key_pair_name](/packer/plugins/builders/vsphere/vsphere-clone#temporary_key_pair_name) will only work
if the template being cloned contains the vApp property `public-keys`.
If using [ssh_private_key_file](/packer/plugins/builders/vsphere/vsphere-clone#ssh_private_key_file), provide
the public key via [configuration_parameters](/packer/plugins/builders/vsphere/vsphere-clone#configuration_parameters) or
[vApp Options Configuration](/packer/plugins/builders/vsphere/vsphere-clone#vapp-options-configuration) whenever the `guestinto.userdata`
